...
async def send_final_callback(session_id: str, state: ConversationState):
    ...
    payload = {
        ...
        "whyScam": "Ultra-advanced 35-layer detection: pattern+semantic+escalation+anomaly+behavioral+URL+normalization+sequencing+contradiction+lifecycle+proximity+momentum analysis",
        "agentNotes": f"Persona:{state.persona.name}|Cat:{state.scam_category.value}|"
                      ...
                      f"Lifecycle:{state.lifecycle_stage}"
    }
